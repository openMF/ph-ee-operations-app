...
public final class UserDataValidator {
    ...
    void validateFieldLevelACL(final String json, PlatformJwt platformJwt) {
        if (!platformJwt.hasAnyPermission("ALL_FUNCTIONS", "UPDATE_USER")) {
    ...
    public void validateForUpdate(final String json, PlatformJwt platformJwt) {
        if (StringUtils.isBlank(json)) {
            throw new InvalidJsonException();
        }
        ...
        throwExceptionIfValidationWarningsExist(dataValidationErrors);
        validateFieldLevelACL(json, platformJwt);
    }
}
