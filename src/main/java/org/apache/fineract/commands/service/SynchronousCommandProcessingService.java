...
import org.apache.fineract.infrastructure.core.domain.OperationsRequestContextHolder;
...
public class SynchronousCommandProcessingService implements CommandProcessingService {
    ...
    private final OperationsRequestContextHolder operationsRequestContextHolder;
    ...
    public CommandProcessingResult executeCommand(final CommandWrapper wrapper, final JsonCommand command,
        ...
        Long commandId = (Long) operationsRequestContextHolder.getAttribute(COMMAND_SOURCE_ID, null);
        ...
        PlatformJwt jwt = context.jwt(wrapper);
        AppUser user = appUserRepository.findAppUserByName(jwt.getUserName());
        ...
        if (isApprovedByChecker || (isMakerChecker && jwt.isCheckerSuperUser())) {
        ...
        try {
            result = commandSourceService.processCommand(findCommandHandler(wrapper), command, commandSource, jwt, isApprovedByChecker,
                    isMakerChecker);
    ...
    private void storeCommandIdInContext(CommandSource savedCommandSource) {
        ...
        operationsRequestContextHolder.setAttribute(COMMAND_SOURCE_ID, savedCommandSource.getId());
    }
    ...
    private void setIdempotencyKeyStoreFlag(boolean flag) {
        operationsRequestContextHolder.setAttribute(IDEMPOTENCY_KEY_STORE_FLAG, flag);
    }
    ...
    @Override
    public boolean validateRollbackCommand(final CommandWrapper commandWrapper, final PlatformJwt jwt) {
        jwt.validateHasPermissionTo(commandWrapper.getTaskPermissionName());
        boolean isMakerChecker = configurationDomainService.isMakerCheckerEnabledForTask(commandWrapper.taskPermissionName());
        return isMakerChecker && !jwt.isCheckerSuperUser();
    }

    protected void publishHookEvent(final String entityName, final String actionName, JsonCommand command, final Object result) {
        final PlatformJwt jwt = context.jwt(CommandWrapper.wrap(actionName, entityName));
        final AppUser user = appUserRepository.findAppUserByName(jwt.getUserName());
        ...
        if (command.json() != null) {
            ...
            reqmap.put("createdBy", user.getId());
            reqmap.put("createdByName", user.getUsername());
            reqmap.put("createdByFullName", user.getDisplayName());
            ...
            final HookEvent applicationEvent = new HookEvent(hookEventSource, serializedResult, user,
