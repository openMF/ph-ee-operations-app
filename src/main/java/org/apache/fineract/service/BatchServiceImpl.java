...
public class BatchServiceImpl implements BatchService {
    ...
    @Override
    public PaymentBatchDetail getPaymentBathDetail(String batchId, String clientCorrelationId, int offset, int limit, String orderBy,
            String sortBy) {
        List<Batch> batchAndSubBatches = batchRepository.findAllByBatchId(batchId);
        ...
        /*
         * updatePaymentDetailBatchInfo(batch, response); for(Batch subBatches : subBatchList) { payeeFspSet = new
         * HashSet<>(); int pageNumber = (offset / limit); Page<Transfer> transferList =
         * transferRepository.findAllByBatchId(batch.getSubBatchId(), new PageRequest(pageNumber, limit));
         * log.info(subBatches.getBatchId()); log.info(transferList.toString()); List<Instruction> instructionList =
         * generateInstructionList(transferList.getContent(),orderBy,sortBy); allInstructions.addAll(instructionList);
         *
         * SubBatchSummary subBatch = updateSubBatchPaymentDetail(batch, response); Long subBatchCount =
         * transferRepository.countAllByBatchId(batch.getSubBatchId()); totalInstruction += subBatchCount;
         * subBatchSummaryList.add(subBatch); }
         */
    ...
    @Override
    public SubBatchSummary getPaymentSubBatchDetail(String batchId, String subBatchId, String clientCorrelationId, int offset, int limit,
