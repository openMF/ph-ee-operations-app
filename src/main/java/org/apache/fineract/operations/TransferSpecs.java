...
public class TransferSpecs {
    ...
    public static <T> Specification<Transfer> multiMatch(SingularAttribute<Transfer, T> attribute1,
            SingularAttribute<Transfer, T> attribute2, T input) {
        return where((root, query, builder) -> builder.or(builder.equal(root.get(attribute1), input),
                builder.equal(root.get(attribute2), input)));
    }

    public static <T> Specification<Transfer> in(SingularAttribute<Transfer, T> attribute, List<T> inputs) {
